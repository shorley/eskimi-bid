...
object DataGenerator {
  ...
  val sites_domains:Seq[(String, String)] = Seq(("FZNd99HW7rcpNzb", "apple.com"), ("TXRIgMS4GivcwcC", "bbc.com"), ("WhL62qMWGRe7ZWI", "cnn.com"), ("MUVG4TqGGFThw4P", "eskimi.com"),
    ("VkyZev7PUyB0Y5q", "facebook.com"), ("XQDkODE92T2KPFt", "google.com"), ("PJ4YCW7bNCVB3cN", "paypal.com"), ("lDwsYTPvjPnntJx", "alibaba.com"), ("JMb73LblxbfrPQ2", "vodafone.com"), ("p653UNliISPWlyu", "netflix.com"))
  ...
  def samplecampaigns(n: Int, fixedcountry:Option[String]=None, minprice:Double, maxprice:Double, startHour: Option[Int] = None, endHour:Option[Int] = None): Seq[Campaign] =
    Seq.tabulate(n)(idx => {
      val bidprice = randomPrice(minprice, maxprice)
      val targeting  = targettingsiteids
      val banners = 0.to(random.nextInt(10)).map(idx2 => {
  ...
  def samplebid(nImpr: Int, fixedcountry:Option[String]=None, pricemin: Double, pricemax: Double): BidRequest = {
    ...
    val impressions:List[Impression] = Seq.tabulate(nImpr)(idx => {
      val bid = randomPrice(pricemin, pricemax)
      val w = randomIntWithSteps(banner_min_w, banner_max_w, 25)
      val wstepfactor = wh_ranges(random.nextInt(wh_ranges.size))
      val min_max_w_opt = Seq((Some(w-wstepfactor), Some(w+wstepfactor), None), (None, None, Some(w)), (Some(w-wstepfactor), Some(w+wstepfactor), Some(w)))
      val min_max_w = min_max_w_opt(random.nextInt(min_max_w_opt.size))
      val h = randomIntWithSteps(banner_min_h, banner_max_h, 10)
      val hstepfactor = wh_ranges(random.nextInt(wh_ranges.size))
      val min_max_h_opt = Seq((Some(h-hstepfactor), Some(h+hstepfactor), None), (None, None, Some(h)), (Some(h-hstepfactor), Some(h+hstepfactor), Some(h)))
      val min_max_h = min_max_h_opt(random.nextInt(min_max_h_opt.size))
      Impression(s"I${randomString(6)}", min_max_w._1, min_max_w._2, min_max_w._3, min_max_h._1, min_max_h._2, min_max_h._3, Some(bid))
    }).toList

    BidRequest(s"${randomString(4)}-${randomString(4)}-${randomString(4)}", Some(impressions), Site(_site._1, _site._2), user_device._1, user_device._2)
  }
