...
@JsonIgnoreProperties(ignoreUnknown = true) case class BidRequest(id: String, imp: Option[List[Impression]], site: Site, user: Option[User], device: Option[Device])
@JsonIgnoreProperties(ignoreUnknown = true) case class Impression(id: String, wmin: Option[Int], wmax: Option[Int], w: Option[Int], hmin: Option[Int], hmax: Option[Int], h: Option[Int], bidFloor: Option[Double])
...
object EskimiBid extends JacksonSupport{
  ...
  def constantCampaigns(): Seq[Campaign] = {
    Seq[Campaign](
      Campaign(id=1, country = "LT", targeting = Targeting(Seq("0006a522ce0f4bbbbaa6b3c38cafaa0f"), startHourOfDay=Some(8), endHourOfDay = Some(21)),
        banners = List(
        ...
        bid = 4.50),
      Campaign(id=2, country = "LT", targeting = Targeting(Seq("0006a522ce0f4bbbbaa6b3c38cafaa0f"), endHourOfDay = Some(21)),
        banners = List(
        ...
        bid = 5.15),
      Campaign(id=3, country = "NG", targeting = Targeting(Seq.empty[String]),
        banners = Nil,
        bid = 2.15)
    )
  }
  ...
  def validateBid(bid: BidRequest)(implicit campaigns: Seq[Campaign]): Option[BidResponse] = {
    ...
    campaigns
      .withFilter(c =>
        bid.device.exists(_.geo.flatMap(_.country.map(_ == c.country)).getOrElse(true)) || bid.user.exists(_.geo.flatMap(_.country.map(_ == c.country)).getOrElse(true))
      )
      .withFilter(_.targeting.targetedSiteIds.contains(bid.site.id))
      .withFilter(c =>
          c.targeting.startHourOfDay.map(hour_of_day >= _).getOrElse(true) && c.targeting.endHourOfDay.map(hour_of_day <= _).getOrElse(true)
      )
      .withFilter(
          c => bid.imp match {
              case None => false
      ...
      .foreach({
        c=>
          val resolved_banners = c.banners.filter ( bnf =>
            within_bid_impr.map(
              { imp =>
                val _exists = (imp.w.exists(_ >= bnf.width) || (imp.wmax.exists(_ >= bnf.width) && imp.wmin.exists(_ <= bnf.width))) &&
                  (imp.h.exists(_ >= bnf.height) || (imp.hmax.exists(_ >= bnf.height) && imp.hmin.exists(_ <= bnf.height)))
          ...
          if(considered_bid_impr.nonEmpty) {
            ...
            resolved_responses += BidResponse(1.toString, bid.id, impr.bidFloor.getOrElse(0.0d), Some(s"${c.id}"), Some(banner))
          }
